...
func TestHistByCustomRange(t *testing.T) {
	convey.Convey("Test HistByCustomRange function", t, func() {
		data := []float64{
			1,
			2, 2.1, 2.5, 2.8,
			3,
		...
		convey.Convey("Test include range for input", func() {
			hist, err := HistByCustomRange(0.5, 20, 2, data)
			convey.So(err, convey.ShouldBeNil)
			convey.So(hist.Count, convey.ShouldEqual, len(data))
			convey.So(hist.Buckets[3].Count, convey.ShouldEqual, 3)
			if err := Fprint(os.Stdout, hist, Linear(5)); err != nil {
				panic(err)
			}
			//0.5-2.5    25%    █████▏  3
		...
		convey.Convey("Test cross range for input", func() {
